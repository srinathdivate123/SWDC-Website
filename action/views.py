...
from authentication.models import Coordinator, Volunteer, Activity, Domain, currentData, Secretary, stats
    ...
    if request.method == "POST":
        ...
        for c in rows:
            ws.append([
                ...
                c.roll,
                c.current_add,
                c.Objective_of_the_Activity,
                c.Description_of_the_Activity,
                c.Benefits_to_Society,
                c.Benefits_to_Self,
                c.Learning_Experiences_challenges,
                c.How_did_it_help_you_to_shape_your_Empathy,
                c.url
                ])
...
def downloadVolunteerCertificatesZip(request):
    ...
    if request.method == 'POST':
        ...
        if len(volunteers) == 0:
            ...
            return redirect("download-volunteer-certificates-zip")
        template_path = "/home/swdc/SWDCWebsite/certificateTemplates/" + act + '/' + act + '_' + year + '_' + semester + ".pdf"
...
def generateIndividualCertificate(request):
    ...
    if request.method == 'POST':
        ...
        template_path = '/home/swdc/SWDCWebsite/certificateTemplates/' + act + '/' + act + '_' + year + '_' + sem + '.pdf'
        if not os.path.exists(template_path):
