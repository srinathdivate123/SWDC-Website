...
from .models import Coordinator, Volunteer, Secretary, Activity, currentData, Departments, Domain, stats
...
def VolunteerRegistrationView(request):
    if request.method == 'GET':
        activities = Activity.objects.filter(enabled=True)
        t_and_c = {}
        for activity in activities:
            t_and_c[activity.name] = activity.t_and_c
        return render(request, 'vregistration.html', {'activities' : activities, 't_and_c': t_and_c, "form":FormWithCaptcha()})
    ...
    if request.method == 'POST':
        ...
        if Volunteer.objects.filter(email = emailVal).exists():
            ...
            if v.registered_academic_year == current.AcademicYear and v.registered_semester == current.Semester:
                if v.verified == 3:
                    ...
                    if not volunteerActivityObj.enabled:
        ...
        status = VolunteerRegistrationComplete(name, emailVal, gender, user_entered_activity, dept, academic_year, div, current_add, prn, roll, contact_num, parent_num, blood_group)
        if not status:
            user.delete()
...
def VolunteerRegistrationComplete(name, email, gender, user_entered_activity, dept, academic_year, div, current_add, prn, roll, contact_num, parent_num, blood_group):
    try:
        currentInfo = currentData.objects.get(index='Current')
        reg = Volunteer.objects.create(vname=name, email=email, gender=gender, activity=user_entered_activity, dept=dept,academic_year=academic_year, registered_academic_year = currentInfo.AcademicYear, registered_semester = currentInfo.Semester, div=div, current_add=current_add, prn=prn, roll=roll, contact_num=contact_num, Cordinator='', parent_num=parent_num, blood_group=blood_group)
        reg.save()
        testGet = Volunteer.objects.get(email=email)
        return True
...
def CoordRegistrationView(request):
    if request.method == "GET":
        ...
        user.is_active = True
        user.save()
        status = CoordinatorRegistrationComplete(name, email, gender, dept, academic_year, div, current_add, prn, roll, contact_num, user_entered_activity, domain, parent_num, blood_group)
        if not status:
            user.delete()
            messages.error(request, 'There was an error! Please try again!')
            return redirect('creg')
...
def CoordinatorRegistrationComplete(name, email, gender, dept, academic_year, div, current_add, prn, roll, contact_num, user_entered_activity, domain, parent_num, blood_group):
    try:
        currentInfo = currentData.objects.get(index='Current')
        ...
        reg.save()
        get = Coordinator.objects.get(email=email)
        return True
    except Exception as errorMessage:
        return False
...
def SecretaryRegistrationView(request):
    if request.method == "GET":
        ...
        user.is_active = True
        user.save()
        status = SecretaryRegistrationComplete(name, email, gender, dept, academic_year, div, current_add, prn, roll, contact_num, domain, activity)
        if not status:
            user.delete()
            messages.error(request, 'There was an error. Please try again!')
            return redirect('sreg')
